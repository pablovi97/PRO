...
el contenido del array de 20 números y mostrar cuáles son los 5 números más pequeños
    ...
    public static int[] obtenerMenores(int size, int array[]) {
        ...
        for (int i = 0; i < resultado.length; i++) {
            ...
            for (int j = resultado.length; j < array.length; j++) {
                int posMayorResultado = pocicionMaximo(resultado);
                if (array[j] < resultado[posMayorResultado] ){
                    resultado[posMayorResultado] = array[j];
                }
            }
    ...
    public static int pocicionMaximo(int array[]) {
        int posMax = 0;
        if (array != null) {
